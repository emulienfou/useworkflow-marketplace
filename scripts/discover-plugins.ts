...
/**
 * Discover all plugin directories recursively
 * Returns an array of plugin paths relative to plugins/ directory
 * e.g. ["communication/resend", "communication/slack"]
 */
    ...
    for (const entry of entries) {
      ...
      try {
        if (statSync(fullPath).isDirectory()) {
          // Check if this directory is a plugin (has index.ts)
          if (existsSync(join(fullPath, "index.ts"))) {
            plugins.push(entryRelativePath);
          } else {
            // If not a plugin, scan deeper (it's a category folder)
            scanDirectory(fullPath, entryRelativePath);
          }
        }
      } catch (e) {
        console.warn(`Warning: Failed to scan ${fullPath}`, e);
      }
    }
  }

  scanDirectory(PLUGINS_DIR, "");
  return plugins.sort();
}
 ...
 */
async function processStepFilesForCodegen(plugins: string[]): Promise<void> {
  ...
  for (const integration of integrations) {
    // Find the plugin path for this integration
    const pluginPath = plugins.find(p => p.endsWith(`/${integration.type}`) || p === integration.type);

    if (!pluginPath) {
      console.warn(`Warning: Could not find plugin path for integration ${integration.type}`);
      continue;
    }
 ...
 */
async function generateStepRegistry(plugins: string[]): Promise<void> {
  const { getAllIntegrations, computeActionId } = await import(
    "@/plugins/registry"
    );
  ...
  const importerEntries = stepEntries
    .flatMap(({ actionId, integration, stepImportPath, stepFunction, pluginPath }) => {
      const entries = [
        `  "${actionId}": {
    importer: () => import("@/plugins/${pluginPath}/steps/${stepImportPath}"),
...
async function main(): Promise<void> {
  ...
  await generateStepRegistry(plugins);
  ...
  await processStepFilesForCodegen(plugins);
